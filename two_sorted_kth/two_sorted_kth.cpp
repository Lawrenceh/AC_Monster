...
int FindKthElm(int A[], int aBeg, int aEnd, int B[], int bBeg, int bEnd, int k)  {  
    ...
    if (A[aMid] < B[bMid])  {  
        if (halfLen > k) return FindKthElm(A, aBeg, aEnd, B, bBeg, bMid - 1, k);  
        else return FindKthElm(A, aMid + 1, aEnd, B, bBeg, bEnd, k - (aMid - aBeg + 1));  
    }  
    else  {  
        if (halfLen > k)  return FindKthElm(A, aBeg, aMid - 1, B, bBeg, bEnd, k);  
...
int main()  {  
    ...
    for (int i = 1; i <= ALen + BLen; ++i)  {  
        if (apos >= ALen)  B[bpos++] = i;  
        else if (bpos >= BLen)  A[apos++] = i;  
        else  {  
            if (rand()%2 == 1)  A[apos++] = i;  
            else  B[bpos++] = i;  
        }  
    }  
