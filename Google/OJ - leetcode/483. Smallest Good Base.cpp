class Solution { /* using discuss sector solution */
public:
    string smallestGoodBase(string n) {
        unsigned long long tn = (unsigned long long)stoll(n);
        unsigned long long x = 1;
        for (int i = 62; i >= 1; i --) { // iterate through max length to min
            if ((x << i) < tn) {
    ...
    unsigned long long mysolve(unsigned long long n, int d) { // binary search find if there exists valid base k
        double tn = (double) n;
